...
class JointTrajectoryExecutor(Node):
    ...
    def __init__(self):
        ...
        # NEW PARAMETER: Maximum allowed velocity for trajectory points (rad/s)
        ...
        self.get_logger().info(f'Max velocity for trajectory generation: {self.max_velocity} rad/s')
    ...
    def create_smooth_trajectory(self, start_pos, end_pos):
        ...
        for i in range(self.num_points):
            ...
            for j in range(len(self.joint_names)):
                ...
                # NEW: Clip the calculated velocity to the specified maximum
                # np.clip(value, a_min, a_max) clips the value to be within the range [a_min, a_max]
                clipped_vel = np.clip(vel, -self.max_velocity, self.max_velocity)
                velocities.append(float(clipped_vel)) # Ensure it's a standard float
